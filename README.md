...
- **No rate limiting on any endpoint.** The search endpoint in particular proxies straight through to Open Food Facts on every request with no throttling, so nothing here protects it (or Open Food Facts) from being hammered by repeated requests.
